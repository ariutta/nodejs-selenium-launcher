...
function downloadChromeDriver(cb) {
  ...
  var real = function() {
    ...
    request('http://chromedriver.storage.googleapis.com/LATEST_RELEASE', function (error, response, body) {
      ...
      http.get(options, function(res) {
          ...
          res.on('data', function(chunk) {
                  ...
                  for (var i = 0; i < zipEntries.length; i++)
                    zip.extractEntryTo(zipEntries[i], './', true, true);
      ...
      /*
      request(chromeDriverDownloadUrl)
        .pipe(fs.createWriteStream(chromeDriverZipfile))

        fs.createReadStream(chromeDriverZipfile)
        .pipe(unzip.Extract({ path: chromeDriverExecutable + 'pipe' }))
        //cb()
      ...
      request(requestOptions)
        .pipe(unzip.Extract({ path: chromeDriverOutfile }));
      //*/
    })
  }
